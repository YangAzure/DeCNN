...
flagList = {}
for i in nei:
    xki[i] = np.zeros((iteration,m,1))
    flagList[i] = np.zeros((iteration,1))
##
...
trainx = [im[x] for x in randomd[(nodeId-1) * portion : nodeId * portion]]
trainy = [train_y[x] for x in randomd[(nodeId-1) * portion : nodeId * portion]]
...
###########################################
    ...
    Q = np.matmul(np.transpose(hnp), trainy)
    omega = inv(1/C*V*I_L + np.matmul(np.transpose(hnp),hnp))
    print omega
